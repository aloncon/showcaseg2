...
export default class Wcan extends React.Component {
    ...
    getUpdatedArray(){
          ...
          for (var i = 0; i < this.props.data_slides.length; i++) {
            var startDate   =   (this.props.data_slides[i][5] !== false ?  new Date(this.props.data_slides[i][5]) : false )
            var endDate     =   (this.props.data_slides[i][6] !== false ?  new Date(this.props.data_slides[i][6]) : false )
    ...
    //control the control nav player classes
          ...
          if(this.state.slideDirection){
              ...
              {
                  next = 1;
              }else if(!this.state.infinite && (next === this.state.slidesNew.length) ){
              ...
              else if(!this.state.infinite && (Number(e) === 1) )
              {
                    console.log('no next');
                    document.getElementById('wcan_previous').className = 'wcan_previous wcan_disable';
                    document.getElementById('wcan_previous').disabled = true;
          ...
          <div style={divStyle}>
            <Slider ref={ c => this.slider = c } beforeChange={ this.changeClass.bind(this)} {...settings}>
                {this.state.slidesNew.map((slide, index) => (
                    <div key={index}>
                        <WcResource><a href={slide[1]} className='wcan_link' title={slide[2]} target="_blank">
                            <WcResource><img src={slide[this.state.changeImgSrc]} alt={slide[2]} title={slide[2]}/></WcResource>
                        </a></WcResource>
                    </div>
                ))}
            </Slider>
